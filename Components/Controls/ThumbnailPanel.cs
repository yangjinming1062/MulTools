...
namespace MulTools.Components
{
    public class ThumbnailPanel : Panel
    {
        ...
        /// </summary>
        protected byte ThumbnailOpacity => (_drawMouseRegions) ? (byte)130 : (byte)255;

        /// <summary>
        /// Gets or sets whether the control should report clicks made on the cloned thumbnail.
        /// </summary>
        public bool ReportThumbnailClicks { get; set; }
        ...
        protected override void WndProc(ref Message m)
        {
            ...
            if (m.Msg == WM.NCHITTEST && m.Result.ToInt32() == HT.CLIENT)//Check whether this is a hit-test on "client" surface
                if (!DrawMouseRegions && !ReportThumbnailClicks)//Check whether clicks must be reported
        ...
        /// </summary>
        public bool IsShowingThumbnail => (_thumbnail != null && !_thumbnail.IsInvalid);
        ...
        private void UpdateThubmnail()
        {
            if (_thumbnail != null && !_thumbnail.IsInvalid)
            {
                ...
                {
                    UnsetThumbnail();//Any error updating the thumbnail forces to unset (handle may not be valid anymore)
                    return;
                }
            }
        }
        ...
        protected virtual void OnRegionDrawn(Rectangle region)
        {
            ...
            RegionDrawn?.Invoke(this, new ThumbnailRegion(region));
        }
        ...
        protected override void OnMouseMove(MouseEventArgs e)
        {
            ...
            {
                _regionLastPoint = e.Location;//Continue drawing
                this.Invalidate();
            }
            else if (DrawMouseRegions && !_drawingRegion)
            {
                _regionLastPoint = e.Location;//Keep track of region start point
                this.Invalidate();
            }
            base.OnMouseMove(e);
        }
        ...
        {
            CloneClick?.Invoke(this, new CloneClickEventArgs(location, buttons, doubleClick));
        }
        #endregion
