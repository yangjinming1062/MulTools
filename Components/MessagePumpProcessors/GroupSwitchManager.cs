...
{
    public class GroupSwitchManager : BaseMessagePumpProcessor
    {
        ...
        public override bool Process(ref Message msg)
        {
            if (_active && msg.Msg == HookMethods.WM_SHELLHOOKMESSAGE)
            {
                ...
                if (hookCode == HookMethods.HSHELL_WINDOWACTIVATED || hookCode == HookMethods.HSHELL_RUDEAPPACTIVATED)
                {
        ...
        private void HandleForegroundWindowChange(IntPtr activeWindow)
        {
            ...
            if (activated == null)
                return;//New foreground window is not tracked
        ...
        protected override void Shutdown() => Disable();
        ...
        {
            //IComparer<WindowHandleWrapper> Members
